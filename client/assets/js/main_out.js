(function(wHandle, wjQuery) {
    ...
    function buildQTree() {
        if (.4 > viewZoom) qTree = null;
            ...
            for (var f = 0; f < nodes.length; ++f) {
                var g = nodes[f];
                a > g.x - g.size && (a = g.x - g.size);
                b > g.y - g.size && (b = g.y - g.size);
                c < g.x + g.size && (c = g.x + g.size);
                d < g.y + g.size && (d = g.y + g.size);
                g.size > e && (e = g.size);
            }
            qTree = new Quad(a - (e + 50), b - (e + 50), c + (e + 50), d + (e + 50));
            for (f = 0; f < nodes.length; f++) {
                if (nodes[f].points.length > 0) {
                    for (var a2 = 0; a2 < nodes[f].points.length; a2++) {
                        qTree.insert(nodes[f].points[a2]);
                    }
                }
            }
        }
    ...
    function drawGameScene() {
        ...
        if (0 < playerCells.length) {
            ...
            a = 0;
            for (var b = 0; b < playerCells.length; ++b) {
                a += playerCells[b].size * playerCells[b].size
            }
            userScore = a;
            posX = 0;
            posY = 0;
            for (b = 0; b < playerCells.length; ++b) {
                posX += playerCells[b].x * playerCells[b].size * playerCells[b].size / a;
                posY += playerCells[b].y * playerCells[b].size * playerCells[b].size / a;
            }
            nodeX = (nodeX + posX) / 2;
            nodeY = (nodeY + posY) / 2;
        } else {
        ...
        chatCanvas && chatCanvas.width && ctx.drawImage(chatCanvas, 10, canvasHeight - chatCanvas.height - 50);
        if (userScore) {
            ...
            ctx.fillRect(10, canvasHeight - 10 - 24 - 10, c + 10, 34);
            ctx.globalAlpha = 1;
    ...
    function wsSend(a) {
        if (ws && 1 == ws.readyState) ws.send(a)
    }
    ...
    function drawTouch(ctx) {
        ...
        ctx.beginPath();
        ctx.arc(canvasWidth - size / 2, canvasHeight - size / 2, size / 2, 0, 2 * Math.PI);
        ctx.fill();
        ctx.beginPath();
        ctx.arc(canvasWidth - size / 2, canvasHeight - 3 * size / 2 - 10, size / 2, 0, 2 * Math.PI);
        ctx.fill();
        ctx.globalAlpha = 1;
    }
    ...
    Cell.prototype = {
        ...
        createPoints: function() {
            ...
            for (var i = 0; i < this.getNumPoints(); i++) {
                var angle = 2 * Math.PI * i / this.getNumPoints();
                this.points.push({ x: this.x + Math.cos(angle) * this.size, y: this.y + Math.sin(angle) * this.size, rl: this.size });
                this.pointsAcc.push(0);
            }
        },
